...
void Move(float time)
{
    if(isRepereSet)
    {
        position.x += cosf(angle) * MAXSPEED * (time/1000);
        position.y += sinf(angle) * MAXSPEED * (time/1000);
    }
}
        ...
        case FollowingSide:
            ...
            if(isWallAhead)
            {
                ...
                else
                {
                    ...
                    float distNewVertexOrigin = sqrtf(newVertexPosition.x * newVertexPosition.x + newVertexPosition.y * newVertexPosition.y);
